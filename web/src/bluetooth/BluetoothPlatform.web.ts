...
export class BluetoothPlatformWeb implements BluetoothPlatform {
    ...
    public async connect(): Promise<void> {
        const device = await navigator.bluetooth
            .requestDevice({filters: [{services: [serviceId]}]})
        const server: BluetoothRemoteGATTServer | undefined = await device.gatt?.connect()
        if (!server) {
            throw new Error("server undefined")
        }
        const service: BluetoothRemoteGATTService = await server.getPrimaryService(serviceId);
        const characteristics: BluetoothRemoteGATTCharacteristic[] = await Promise.all([
